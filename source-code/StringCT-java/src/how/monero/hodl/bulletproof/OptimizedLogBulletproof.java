...
public class OptimizedLogBulletproof
{
    ...
    public static InnerProductProofTuple InnerProductArgument(Scalar[] a, Scalar[] b, Curve25519Point[] Gprime, Curve25519Point[] Hprime, Curve25519Point u, Scalar seed)
    {
    ...
    public static boolean InnerProductArgumentVerify(InnerProductProofTuple proof, Curve25519Point P, Curve25519Point[] Gprime, Curve25519Point[] Hprime, Scalar y, Scalar z, Scalar mu, Scalar x_ip, Scalar t) {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            hScalar = hScalar.sub(z.mul(y.pow(i)).add(z.sq().mul(Scalar.TWO.pow(i))));

            // Now compute the basepoint's scalar multiplication
            // Each of these could be written as a multiexp operation instead
            InnerProdG = InnerProdG.add(Gprime[i].scalarMultiply(gScalar));
            InnerProdH = InnerProdH.add(Hprime[i].scalarMultiply(hScalar));
        }
    ...
    public static ProofTuple PROVE(Scalar v, Scalar gamma)
    {
        ...
        InnerProductProofTuple innerProductProof = InnerProductArgument(l,r,Gprime,Hprime,H.scalarMultiply(x_ip),x_ip);
    ...
    public static boolean VERIFY(ProofTuple proof)
    {
        ...
        if(!InnerProductArgumentVerify(innerProductProof,P,Gprime,Hprime,y,z,proof.mu,x_ip,proof.t))
        	return false;
